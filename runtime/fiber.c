...
int caml_try_realloc_stack(asize_t required_space)
{
   ...
   * on existing stacks. */
  {
    ...
    if (!has_c_stack_link) {
      /* No c_stack_link for this stack - it's a pure-OCaml fiber.
         Walk from SP to update frame pointers. Use First_frame to adjust
         for any architecture-specific offset in how Stack_sp is saved
         (e.g., Power saves SP - 32, so First_frame adds 32 back). */
      struct stack_frame {
        struct stack_frame* prev;
      };

      struct stack_frame* fp = (struct stack_frame*)First_frame((char*)new_stack->sp);
    ...
    for (struct c_stack_link *link = Caml_state->c_stack;
      ...
      if (link->stack == old_stack) {
        ...
        /* Get the OCaml frame that was active when switching to C.
           On most architectures (x86_64, ARM64), the frame is 16 bytes below
           the c_stack_link (sizeof prev + sizeof retaddr). The c_stack_link
           is on the C stack, so we start there and the first back_chain
           takes us into the OCaml stack.
           On Power, c_stack_link has 32 bytes of reserved space at offset 0,
           so we access the OCaml frame via link->sp. Since link->sp still
           contains the old address, we adjust by delta to get the new address
           so we walk through and update the new stack, not the old one. */
#if defined(TARGET_power)
        struct stack_frame* fp = (struct stack_frame*)((char*)link->sp + delta);
#else
        struct stack_frame* fp = (struct stack_frame*)((char*)link - 16);
