  ...
  for i = 0 to String.length s - 1 do
    ...
    | _ ->
      if c = Compilenv.symbol_separator then
        ...
        Printf.fprintf !output_channel "$%03d" (Char.code c)
  done
      ...
      if line > 0 then begin (* PR#6243 *)
        let file_num =
          ...
          with Not_found ->
            ...
            file_num in
        loc_emitter ~file_num ~file_name ~line ~col;
      end
  end
...
let emit_debug_info dbg =
  emit_debug_info_gen dbg (fun ~file_num ~file_name ->
      ...
      emit_string_literal "/Users/tsmc/projects/ocaml"; emit_char '\t';
      emit_string_literal file_name; emit_char '\n';
    )
    (fun ~file_num ~file_name ~line ~col ->
       emit_string "\t.loc\t";
       emit_int file_num; emit_char '\t';
       emit_int line; emit_char '\t';
       emit_int col; emit_char '\t';
