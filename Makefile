...
ifneq ($(DTRACE),)
runtime_BUILT_HEADERS += runtime/caml/ocaml_probes.h
endif
...
# runtime/caml/ocaml_probes.h is generated from ocaml_probes.d on macOS/FreeBSD
# when USDT probes are enabled. On Linux, SystemTap SDT is used directly.
ifneq ($(DTRACE),)
runtime/caml/ocaml_probes.h : runtime/caml/ocaml_probes.d
	$(V_GEN)cd runtime/caml && $(DTRACE) -h -s ocaml_probes.d -o ocaml_probes.h
endif

# On FreeBSD, dtrace -G must be run to generate an object file that embeds
# the DOF section (needed for probe discovery). dtrace -G also patches the
# input .o files in-place, replacing probe calls with NOPs.
# On macOS, the compiler/linker handles probes natively; on Linux, SystemTap
# SDT is used directly. So this block is FreeBSD-only.
ifeq "$(SYSTEM)" "freebsd"

# $(1) = object suffix (b, n, bd, nd, bi, ni, bpic, npic)
# $(2) = C source list variable (runtime_BYTECODE_C_SOURCES, etc.)
define DTRACE_PROBE_OBJECT
runtime/ocaml_probes.$(1).$$(O): runtime/caml/ocaml_probes.d \
    $$($(2):.c=.$(1).$$(O))
	$$(V_GEN)$$(DTRACE) -G \
	  -s runtime/caml/ocaml_probes.d \
	  -o $$@ $$($(2):.c=.$(1).$$(O))
endef

$(eval $(call DTRACE_PROBE_OBJECT,b,runtime_BYTECODE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,bd,runtime_BYTECODE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,bi,runtime_BYTECODE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,bpic,runtime_BYTECODE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,n,runtime_NATIVE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,nd,runtime_NATIVE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,ni,runtime_NATIVE_C_SOURCES))
$(eval $(call DTRACE_PROBE_OBJECT,npic,runtime_NATIVE_C_SOURCES))

# dtrace -G modifies input .o files in-place, so archives must be built
# AFTER the probe object rule runs. Use order-only prerequisites to
# enforce this ordering without adding the probe object to the archive.
runtime/libcamlrun.$(A): | runtime/ocaml_probes.b.$(O)
runtime/libcamlrun_non_shared.$(A): | runtime/ocaml_probes.b.$(O)
runtime/libcamlrund.$(A): | runtime/ocaml_probes.bd.$(O)
runtime/libcamlruni.$(A): | runtime/ocaml_probes.bi.$(O)
runtime/libcamlrun_pic.$(A): | runtime/ocaml_probes.bpic.$(O)
runtime/libcamlrun_shared.$(SO): | runtime/ocaml_probes.bpic.$(O)
runtime/libasmrun.$(A): | runtime/ocaml_probes.n.$(O)
runtime/libasmrund.$(A): | runtime/ocaml_probes.nd.$(O)
runtime/libasmruni.$(A): | runtime/ocaml_probes.ni.$(O)
runtime/libasmrun_pic.$(A): | runtime/ocaml_probes.npic.$(O)
runtime/libasmrun_shared.$(SO): | runtime/ocaml_probes.npic.$(O)

# Link probe objects directly into runtime executables so the DOF section
# is embedded in the final binary (required for dtrace probe discovery).
# Individual .o files are always fully included by the linker, unlike
# archive members which are only pulled in to resolve undefined symbols.
runtime/ocamlrun$(EXE): runtime/ocaml_probes.b.$(O)
runtime/ocamlruns$(EXE): runtime/ocaml_probes.b.$(O)
runtime/ocamlrund$(EXE): runtime/ocaml_probes.bd.$(O)
runtime/ocamlruni$(EXE): runtime/ocaml_probes.bi.$(O)

endif # ifeq "$(SYSTEM)" "freebsd"
